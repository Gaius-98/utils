...
const knownHeight = computed(() =>
  allHeightList.value.reduce((p, c) => {
    p += c;
    return p;
  }, 0)
);
...
const onScroll = () => {
  ...
  if (scrollTop.value + height > knownHeight.value) {
    ...
    if (knownHeight.value - res >= height) {
      ...
      endIndex.value =
        allHeightList.value.length +
        Math.ceil(height - (knownHeight.value - res) / preHeight.value);
    }
  }
...
const reduceHeight = (idx: number) => {
  ...
  return (
    knownHeight.value + (idx - allHeightList.value.length) * preHeight.value
  );
};
onMounted(() => {
  onScroll();
